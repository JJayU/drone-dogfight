...
class ControlNode(Node):
    def __init__(self):
        ...
        self.stabilize_duration = 4.0
        self.target_duration = 1.0
        
        # Punkt docelowy i zapamiętany kąt yaw
        self.target_point = {'x': 2.0, 'y': 2.0, 'z': 1.5}
        self.target_yaw = None  # Początkowo None, zostanie ustawiony przy pierwszym obrocie
        ...
        self.stable_setpoints = {
            ...
            'y': 1.0,
            'z': 1.0,
            'yaw': 0.0  # To będzie aktualizowane po pierwszym obrocie
        }
        ...
        # Position control - reduced gains for smoother response
        self.x_pos_pid = PID(kp=0.6, ki=0.01, kd=0.8, setpoint=1.0, name="X Position")
        self.y_pos_pid = PID(kp=0.6, ki=0.01, kd=0.8, setpoint=1.0, name="Y Position")

        # Attitude control - increased D gain for better damping
        self.roll_pid = PID(kp=2.5, ki=0.005, kd=0.8, setpoint=0.0)
        self.pitch_pid = PID(kp=2.5, ki=0.005, kd=0.8, setpoint=0.0)
        self.yaw_pid = PID(kp=1.0, ki=0.05, kd=0.6, setpoint=0.0)

        # Height control - smoother response
        self.height_pid = PID(kp=6.0, ki=0.01, kd=2.5, setpoint=1.0)
        
        # Increased update rate for better control
        self.dt = 0.004  # 4ms for faster response
    ...
    def calculate_target_orientation(self):
        """Oblicza docelową orientację drona względem punktu docelowego"""
        dx = self.target_point['x'] - self.x
        dy = self.target_point['y'] - self.y
        dz = self.target_point['z'] - self.z
        
        # Jeśli nie mamy jeszcze ustalonego yaw, oblicz go
        if self.target_yaw is None:
            self.target_yaw = np.arctan2(dy, dx)
            # Zaktualizuj także setpoint dla stabilizacji
            self.stable_setpoints['yaw'] = self.target_yaw
            print(f"\n=== Ustalono docelowy kąt yaw: {self.target_yaw:.2f} rad ===")
        
        # Używamy zapisanego yaw zamiast ciągłego przeliczania
        target_yaw = self.target_yaw
        
        # Oblicz pitch i roll jak wcześniej
        target_pitch = np.arctan2(dx, dz) * 0.5
        target_roll = np.arctan2(-dy, dz) * 0.5
    ...
    def transform_setpoints(self, x_setpoint, y_setpoint):
        """
        Transformuje setpointy x,y z układu globalnego do lokalnego układu drona
        biorąc pod uwagę aktualny yaw
        """
        # Oblicz różnicę między aktualną pozycją a setpointem
        dx = x_setpoint - self.x
        dy = y_setpoint - self.y
        
        # Obróć wektor o aktualny kąt yaw drona
        cos_yaw = np.cos(-self.yaw)  # Ujemny yaw bo obracamy w przeciwnym kierunku
        sin_yaw = np.sin(-self.yaw)
        
        # Transformacja do układu lokalnego drona
    ...
    def control_update(self):
        ...
        if self.current_mode == "init":
            # W trybie init najpierw obracamy się w stronę celu
            if self.target_yaw is None:
                _, _, _ = self.calculate_target_orientation()  # To ustawi target_yaw
            
            # Transformuj setpointy względem aktualnego yaw
            x_local, y_local = self.transform_setpoints(
                self.stable_setpoints['x'], 
                self.stable_setpoints['y']
            )
            
            # Aktualizuj setpointy PID z lokalnymi współrzędnymi
            self.x_pos_pid.setpoint = x_local
            self.y_pos_pid.setpoint = y_local
            
            # Standardowa kontrola pozycji
            desired_pitch = self.x_pos_pid.update(0.0, self.dt)  # 0.0 bo używamy lokalnych współrzędnych
            desired_roll = -self.y_pos_pid.update(0.0, self.dt)
        ...
        elif self.current_mode == "stabilize":
            # Transformuj setpointy względem aktualnego yaw
            x_local, y_local = self.transform_setpoints(
                self.stable_setpoints['x'], 
                self.stable_setpoints['y']
            ...
            desired_pitch = self.x_pos_pid.update(0.0, self.dt)
            desired_roll = -self.y_pos_pid.update(0.0, self.dt)
            ...
            # Transformuj punkt docelowy względem aktualnego yaw
            x_local, y_local = self.transform_setpoints(
                self.target_point['x'], 
                self.target_point['y']
            )
            
            # Oblicz kąty na podstawie lokalnych współrzędnych
            target_pitch = np.arctan2(x_local, self.target_point['z'] - self.z) * 0.5
            target_roll = np.arctan2(-y_local, self.target_point['z'] - self.z) * 0.5
        ...
        # Reszta control_update pozostaje bez zmian
