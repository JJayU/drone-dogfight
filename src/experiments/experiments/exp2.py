...
class Experiment5Node(Node):
    def __init__(self):
        ...
        self.target_center_x = 5.0
        self.target_center_y = 5.0
        ...
        self.follow_distance = 2.5
        self.follow_height_offset = 0.5
    ...
    def calculate_target_position(self, elapsed_time):
        ...
        target_x = self.target_center_x + scale * np.sin(t) / (1 + np.cos(t)**2)
        target_y = self.target_center_y + scale * np.sin(t) * np.cos(t) / (1 + np.cos(t)**2)
        height_variation = self.height_amplitude * np.sin(0.5 * t)
        target_z = self.target_base_height + height_variation

        dt = 0.01
        t_next = t + dt
        next_x = self.target_center_x + scale * np.sin(t_next) / (1 + np.cos(t_next)**2)
        next_y = self.target_center_y + scale * np.sin(t_next) * np.cos(t_next) / (1 + np.cos(t_next)**2)
    ...
    def calculate_drone_desired_position(self):
        ...
        aim_yaw = np.arctan2(self.target_y - desired_y, self.target_x - desired_x)
    ...
    def update(self):
        ...
        if self.experiment_started:
            ...
            desired_pos.pose.position.z = 1.0
